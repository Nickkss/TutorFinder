...
class TutorsView(View):
    def filter_tutors(self, request):
        ...
        q = get.get('q', None)
        if q:
            tutors = tutors.filter(Q(name__contains=q))
            for tutor in tutors:
        ...
        if any(new_days) or language or all([charges_gte, charges_lte]) or q:#if any(days):
            return (new_tutors, new_days, (charges_gte, charges_lte), language, _lang_level, q)
        else:
            return (list(tutors), None, None, None, None, None)
