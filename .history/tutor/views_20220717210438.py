...
class TutorsView(View):
    def filter_tutors(self, request):
        ...
        print(any(days) or language or all([charges_gte, charges_lte]))
        if any(days) or language or all([charges_gte, charges_lte]):#if any(days):
            return (new_tutors, new_days, (charges_gte, charges_lte), language, _lang_level)
        else:
            return (list(tutors), None, None, None)
